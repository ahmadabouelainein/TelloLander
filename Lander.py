...
def memory_limit():
    ...
    resource.setrlimit(resource.RLIMIT_AS, (get_memory()*1024*1, hard))
    soft, hard = resource.getrlimit(resource.RLIMIT_AS)
    print('Memory:',soft)
...
# Read first frame.
    ...
    frame, bbox = read_frame(frame_read)
    print(bbox)
    ...
    while True:
        ...
        if ok:
            ...
            Ysend=int(25*Yerr/((p1[1] + p2[1]) / 2)) #X distance to the center in cm
            Xsend=int(20*Xerr/((p1[0] + p2[0]) / 2)) #Y distance to the center in cm
            print("Error in X: ", Ysend)
            print("Error in y: ", Xsend)

            if drone.get_height() <= 30:               #Landing
                drone.land()
            elif abs(Xsend) <= 3 and abs(Ysend) <= 3:   #Descent
